...
const EducationCard = ({ limit ,theme}) => {
  const [podcasts, setPodcasts] = useState([]);
   ...
   const getallpodcasts = async () => {
     setLoading(true);
     try {
       const res = await fetch(`http://localhost:4000/api/card/category/filter?category=Education`);
       const data = await res.json();
       setPodcasts(limit ? data.slice(0, limit) : data);
     } catch (error) {
       console.error("Error fetching Education podcasts:", error);
     }
     setLoading(false);
   };
