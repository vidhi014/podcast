...
const HealthCard = ({ limit ,theme}) => {
  ...
  const handleDeleteClick = () => {
   ...
   const getallpodcasts = async () => {
     setLoading(true);
     try {
       const res = await fetch(`http://localhost:4000/api/card/category/filter?category=Health`);
       const data = await res.json();
       setPodcasts(limit ? data.slice(0, limit) : data);
     } catch (error) {
       console.error("Error fetching Health podcasts:", error);
     }
     setLoading(false);
   };
